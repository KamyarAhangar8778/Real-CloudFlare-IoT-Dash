/**
 * @file AutomationsTabsSlider.tsx
 * @description Slider component for tab navigation inside AutomationsDrawer matching SettingsDrawer style.
 */

import React from "react";
import { motion } from "motion/react";
import { ChevronLeft, ChevronRight } from "lucide-react";
import { AUTOMATIONS_TABS } from "./types";

interface AutomationsTabsSliderProps {
  activeTab: string;
...
/**
 * AutomationsTabsSlider renders tab slider controls and active tab indicators for AutomationsDrawer.
 */
export function AutomationsTabsSlider({
  activeTab,
  setActiveTab,
  animationsEnabled = true,
  isDark,
}: AutomationsTabsSliderProps) {
  const handleNext = () => {
    const currIndex = AUTOMATIONS_TABS.findIndex((t) => t.id === activeTab);
    const nextIndex = (currIndex + 1) % AUTOMATIONS_TABS.length;
    setActiveTab(AUTOMATIONS_TABS[nextIndex].id);
  };

  const handlePrev = () => {
    const currIndex = AUTOMATIONS_TABS.findIndex((t) => t.id === activeTab);
    const prevIndex = (currIndex - 1 + AUTOMATIONS_TABS.length) % AUTOMATIONS_TABS.length;
    setActiveTab(AUTOMATIONS_TABS[prevIndex].id);
  };

  const currIndex = AUTOMATIONS_TABS.findIndex((t) => t.id === activeTab);
  const activeTabObj = AUTOMATIONS_TABS[currIndex] || AUTOMATIONS_TABS[0];
