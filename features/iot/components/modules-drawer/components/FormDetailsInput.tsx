...
interface FormDetailsInputProps {
  ...
  offLabel?: string;
  setOffLabel?: (val: string) => void;
  onLabel?: string;
  setOnLabel?: (val: string) => void;
}
  ...
  return (
    <>
      <div className="space-y-1.5">
        ...
        <input
          type="text"
          placeholder="مثال: رله پمپ آب یا قفل حیاط"
          value={customTitle}
        ...
        <div className="grid grid-cols-2 gap-2 pt-2 border-t border-[var(--border-color)]">
          <div className="space-y-1 text-right">
            <label className="text-[10px] theme-text-tertiary font-bold block">
              نوشته حالت خاموش (LOW):
            </label>
            <input
              type="text"
              placeholder="مثال: در بسته / LOW"
              value={offLabel}
              onChange={(e) => setOffLabel(e.target.value)}
              className="w-full h-9 px-3 text-xs bg-[var(--bg-main)] text-[var(--text-primary)] border border-[var(--border-color)] rounded-xl focus:border-[var(--accent3)] outline-none font-sans text-right placeholder:text-[var(--text-muted)]"
            />
          </div>
          <div className="space-y-1 text-right">
            <label className="text-[10px] theme-text-tertiary font-bold block">
              نوشته حالت روشن (HIGH):
            </label>
            <input
              type="text"
              placeholder="مثال: در باز / HIGH"
              value={onLabel}
              onChange={(e) => setOnLabel(e.target.value)}
              className="w-full h-9 px-3 text-xs bg-[var(--bg-main)] text-[var(--text-primary)] border border-[var(--border-color)] rounded-xl focus:border-[var(--accent3)] outline-none font-sans text-right placeholder:text-[var(--accent4)]/70"
            />
          </div>
        </div>
      )}

      <IconPicker label="آیکون سگمنت:" selectedIcon={segmentIcon} onSelectIcon={setSegmentIcon} />
          ...
          <div className="flex items-center justify-end gap-3 mb-2 px-1">
            <label className="flex items-center gap-1.5 cursor-pointer">
              <input
                ...
                className="accent-[var(--accent3)] w-3 h-3"
              />
        ...
        {!isNewGroup && existingGroups.length > 0 ? (
          ...
          >
            {existingGroups.map((group) => (
              <option key={group} value={group} className="bg-[var(--bg-main)]">
                {group}
              </option>
            ))}
          </select>
