...
export default function SortableSegmentCard(props: SortableSegmentCardProps) {
  const { segment, parentGroupsCols = 1, groupMaxCols = 3, groupItemsCount = 1, animationsEnabled = true } = props;
  ...
  const densityFactor = effectiveGroupCols * parentGroupsCols;
  const isUltraCompact = densityFactor >= 6;
  const isCompact = densityFactor === 4 || densityFactor === 3;

  if (isDragging) {
    return (
      <div ref={setNodeRef} style={style} className={`touch-none relative h-full ${isSettingsOpen ? '!basis-full !max-w-full' : ''}`}>
        <div
  ...
  return (
    <div ref={setNodeRef} style={style} className={`touch-none relative h-full transition-all duration-300 ${isSettingsOpen ? 'z-40' : ''}`}>
      <ActiveCard
        {...props}
        isCompact={isCompact}
        isUltraCompact={isUltraCompact}
