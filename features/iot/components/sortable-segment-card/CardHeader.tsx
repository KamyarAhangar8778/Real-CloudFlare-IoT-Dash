...
import { X, GripVertical, Cpu, Settings } from "lucide-react";
...
export default function CardHeader({
  ...
  const isSmall = isUltraCompact || isCompact;
  ...
  React.useLayoutEffect(() => {
    if (isSettingsOpen && menuRef.current && buttonRef.current) {
      ...
      const segmentContainer = button.closest('.touch-none.w-full.relative.h-full') as HTMLElement;
      const groupContainer = button.closest('.group\\/group-card') as HTMLElement;
      
      if (segmentContainer && groupContainer) {
        ...
        const offset = groupCenter - segmentCenter;
  ...
  const renderControls = () => {
    ...
    return (
      <div
        className="flex items-center bg-slate-200/90 dark:bg-slate-950 border border-slate-300/85 dark:border-slate-800 p-[3px] rounded-full shadow-sm relative"
  ...
  return (
    <div
      ...
      className={`flex items-center border-b border-[var(--border-color)] bg-slate-500/[0.05] dark:bg-black/25 ${
        isSmall ? "justify-center p-2 cursor-grab active:cursor-grabbing" : "justify-between p-4"
      }`}
    >
      <div className={`flex items-center ${isSmall ? "w-full justify-center" : "gap-1.5 md:gap-2"}`}>
        {!isSmall && (
          <>
            <button
        ...
        {isSmall && (
          <div className="relative flex justify-center w-full">
            <button
              ...
              className={`px-5 py-1 bg-slate-200/90 dark:bg-slate-950 border border-slate-300/85 dark:border-slate-800 transition-all cursor-pointer rounded-full shadow-sm flex items-center justify-center ${
                isSettingsOpen ? "border-[var(--accent3)] text-[var(--accent3)] shadow-[0_0_8px_rgba(255,165,0,0.3)]" : "text-slate-700 dark:text-gray-300 hover:border-[var(--accent3)] hover:text-[var(--accent3)] hover:bg-slate-300/50 dark:hover:bg-slate-800/50"
              }`}
              title="تنظیمات سگمنت"
              onPointerDown={(e) => e.stopPropagation()}
            >
            ...
            {isSettingsOpen && (
              <div 
                ref={menuRef}
                className="absolute top-0 w-[280px] sm:w-[340px] bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl border border-slate-200 dark:border-slate-700 rounded-2xl shadow-[0_20px_50px_-12px_rgba(0,0,0,0.5)] p-4 z-[100] flex flex-col gap-4 animate-in fade-in zoom-in-95 duration-200"
              ...
              >
                <div className="flex items-center justify-between border-b border-slate-200/60 dark:border-slate-700/60 pb-3">
                  ...
                  <button
                    onClick={() => onRemove(segment.id)}
                    className="p-1.5 bg-red-500/10 text-red-600 hover:bg-red-500 hover:text-white transition-all cursor-pointer rounded-lg shadow-sm"
                    title="حذف سگمنت"
                  >
                    <X className="w-4 h-4" />
                  </button>
                ...
                {onUpdateSegmentMode && segment.type !== "input" && (
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-bold text-slate-600 dark:text-slate-400">حالت عملکرد:</span>
                    {renderControls()}
                  </div>
                )}
              </div>
      ...
      {!isSmall && (
        <div className="flex items-center gap-2">
          <div className="text-right">
            <span className="block font-sans font-extrabold theme-text-primary group-hover:text-[var(--accent3)] transition-colors text-xs">
              {segment.title}
            </span>
          </div>
          <div
